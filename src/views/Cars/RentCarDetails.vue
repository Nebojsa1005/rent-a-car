...
export default {
  ...
  methods: {
    ...mapActions(["order", "updateCar", "updateCustomer"]),
    setCurrentCustomer(customerId) {
      let currentCustomer = this.customers.filter(
    ...
    setConfirmedData(data) {
      ...
      let daysToBeUsed = new Date(
        this.returningDate.getTime() - this.startingDate.getTime()
      );
      if (this.customer.lastRent) {
        let timeDiffrence = new Date(
          new Date().getTime() - new Date(this.customer.lastRent).getTime()
        );
        // 1 day has 86400000 miliseconds
        this.customerBonusPeriod =
          timeDiffrence.getTime() / 86400000 < 60 &&
          this.customer.rentedVehicles > 3;
      }
      ...
      this.totalPrice = +(
        this.wantedCar.price -
        (this.wantedCar.price * this.discount) / 100
      ).toFixed(2);
    },
    async orderNow() {
  ...
  computed: {
    ...
    validDates() {
      return (
        ...
        new Date(this.returningDate).getTime()
      );
    },
  },
};
